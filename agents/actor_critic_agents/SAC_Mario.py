...
class SAC_Mario(Base_Agent_Mario):
    ...
    def reset_game(self):
        ...
        Base_Agent_Mario.reset_game(self)
    ...
    def step(self):
        ...
        while not self.done and self.episode_step_number_val < self.max_steps_per_episode:
            ...
            if self.time_for_critic_and_actor_to_learn():
                for _ in range(self.hyperparameters["learning_updates_per_learning_session"]):
                    self.learn()
            mask = False if self.episode_step_number_val >= self.max_steps_per_episode else self.done # self.environment.spec.max_episode_steps else self.done
            if not eval_ep: self.save_experience(experience=(self.state, self.action, self.reward, self.next_state, mask))
            self.state = self.next_state
            self.global_step_number += 1
            # print(self.episode_step_number_val, self.global_step_number)
